...
`src/data/generated/results.json`. You can run it alone at any time:
...
The workflow gates deployment on `build:data`, `test`, `typecheck` and `lint`, so a broken build
never reaches the published site. It also fails if the committed `results.json` is out of date
with `results/` — regenerate and commit rather than letting the two drift.
...
`viz/.gitignore` and the repository-root `.gitignore` were **not modified**.
